...
public class UICamera : MonoBehaviour
{
    ...
    static public bool IsInverseTouchActive = false;
    static public bool IsRaycastingActive = true;
    static public readonly string InverseTouchLayerName = "InverseTouch";
    ...
    static public bool Raycast (Vector3 inPos)
    {
        if (IsRaycastingActive)
        {
            ...
            {
                UICamera cam = list.buffer[i];
                RaycastReturnType returnType = RaycastReturnType.Null;
                if (IsInverseTouchActive)
                {
                    int previousCameraCullingMask = cam.cachedCamera.cullingMask;
                    LayerMask previousCameraEventMask = cam.eventReceiverMask;
                    
                    cam.cachedCamera.cullingMask = 1 << LayerMask.NameToLayer(InverseTouchLayerName);
                    cam.eventReceiverMask = 1 << LayerMask.NameToLayer(InverseTouchLayerName);
                    
                    returnType = RaycastForCamera(cam, inPos);
                    
                    cam.cachedCamera.cullingMask = previousCameraCullingMask;
                    cam.eventReceiverMask = previousCameraEventMask;
                    
                    if (returnType == RaycastReturnType.True)
                    {
                        returnType = RaycastForCamera(cam, inPos);
                        if (returnType == RaycastReturnType.True)
                            return true;
                    }
                    if (returnType == RaycastReturnType.Continue)
                        continue;
                }
                else
                {
    ...
    static public RaycastReturnType RaycastForCamera (UICamera cam, Vector3 inPos)
	{			
		// Skip inactive scripts
        if (!cam.enabled || !NGUITools.GetActive(cam.gameObject)) return RaycastReturnType.Continue;

		// Convert to view space
		currentCamera = cam.cachedCamera;
		Vector3 pos = currentCamera.ScreenToViewportPoint(inPos);
        if (float.IsNaN(pos.x) || float.IsNaN(pos.y)) return RaycastReturnType.Continue;

		// If it's outside the camera's viewport, do nothing
        if (pos.x < 0f || pos.x > 1f || pos.y < 0f || pos.y > 1f) return RaycastReturnType.Continue;
		...
		if (cam.eventType == EventType.World_3D)
		{
			if (Physics.Raycast(ray, out lastHit, dist, mask))
			{
                ...
                return RaycastReturnType.True;
			}
            return RaycastReturnType.Continue;
		}
		else if (cam.eventType == EventType.UI_3D)
		{
			...
			if (hits.Length > 1)
			{
				for (int b = 0; b < hits.Length; ++b)
				{
					...
					if (w != null)
					{
                        if (!w.isVisible) return RaycastReturnType.Continue;
                        if (w.hitCheck != null && !w.hitCheck(hits[b].point)) return RaycastReturnType.Continue;
					}
					else
					{
						UIRect rect = NGUITools.FindInParents<UIRect>(go);
                        if (rect != null && rect.finalAlpha < 0.001f) return RaycastReturnType.Continue;
					}
			...
			else if (hits.Length == 1)
			{
				...
				{
                    if (!w.isVisible) return RaycastReturnType.Continue;
                    if (w.hitCheck != null && !w.hitCheck(hits[0].point)) return RaycastReturnType.Continue;
				}
				else
				{
					UIRect rect = NGUITools.FindInParents<UIRect>(go);
			...
			if (m2DPlane.Raycast(ray, out dist))
			{
				...
				if (hits.Length > 1)
				{
					for (int b = 0; b < hits.Length; ++b)
					{
						...
						{
                            if (!w.isVisible) return RaycastReturnType.Continue;
                            if (w.hitCheck != null && !w.hitCheck(lastWorldPosition)) return RaycastReturnType.Continue;
						}
						else
						{
							UIRect rect = NGUITools.FindInParents<UIRect>(go);
...
#endif
        return RaycastReturnType.False;
	}
